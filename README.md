...
Effect	LeftSlide(기본값) : 왼쪽에서 오른쪽으로 슬라이드
...
reloadInterval	min(기본값) : 20 초 
max : 120 초
enableDefaultBannerAd	false : 디폴트배너 미노출 
true : 디폴트배너 노출
bannerHeight	Proportional(기본값) : 디바이스 긴 방향 해상도의 10
Fixed : 48dp
threadPriority	스레드 우선 순위 지정 : 1~10(기본값 : 5)
[XML 방식 끝]

B.	JAVA 방식 base [자세한 내용은 ‘CaulyExample’ 참조]
1.	위치 : ‘ res  layout  ‘광고 삽입할 부분’.xml 
...
-	배너 광고
1.	위치 : src >> ‘package name’ >> ‘광고를 붙일 Activity’.java
...
샘플 appcode
배너광고 : CAULY, CAULY-RICHADTEST, CAULY-MRATEST-AD4, CAULY-3DTEST
전면광고 : CAULY, CAULY-VIDEOTEST 
Effect()	LeftSlide(기본값) : 왼쪽에서 오른쪽으로 슬라이드
RightSlide : 오른쪽에서 왼쪽으로 슬라이드
TopSlide : 위에서 아래로 슬라이드
BottomSlide : 아래서 위로 슬라이드
...
Circle : 한 바퀴 롤링
None : 애니메이션 효과 없이 바로 광고 교체
reloadInterval()	min(기본값) : 20초)
max : 120 초
dynamicReloadInterval()	true(기본값) : 광고에 따라 노출 주기 조정할 수 있도록 하여 광고 수익 상승 효과 기대
false : reloadInterval 설정 값으로 Rolling
bannerHeight()	Proportional(기본값) : 디바이스 긴방향 해상도의 10%
Fixed : 48dp
enableDefaultBannerAd ()	false : 디폴트배너 미노출 
true : 디폴트배너 노출
threadPriority()	스레드 우선 순위 지정 : 1~10(기본값 : 5)
...
8.	파싱방법
①	List<HashMap<KEY,VALUE>> 방식으로 가져오는 경우,
mCaulyAdView.getAdsList();
②	Raw JSON String을 직접가져 경우
mCaulyAdView.getJsonString();
